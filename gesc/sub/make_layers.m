...
% MED(1) contains depth, Vs, Vp, Rho and depth of interfaces and solid-water
...
MED(1).solid=zeros(Nl,1);
MED(1).solid(1)=1;
for ii=1:Nl-1;
    if beta(ii)~=0; MED(1).solid(Nl-ii+1)=1; end;
end
...
for i=2:MED(1).Nl
    ...
    MED(MED(1).Nl-i+2).zz(1) = min(MED(1).inter(i),MED(1).inter(i-1));
    MED(MED(1).Nl-i+2).zz(end) = max(MED(1).inter(i),MED(1).inter(i-1));
...
ylim([0 max(H)]);ylabel('Depth (km)');legend('Vs (km/s)','Vp (km/s)','density (kg/dm^3)','Location','NorthOutside')
