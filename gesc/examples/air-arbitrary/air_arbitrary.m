% Script to calculate the surface wave eigenfunctions from a general
% velocity profile. User has to pick the right interfaces to respect
% smoothness of the medium using get_layers
% Type "help get_layers" for the interactive layer clicking/picking

% in this example, the file profile.dat that contains the elastic properties with depth
% are formated as follow either meters (and m/s) or in km (and km/s) in :
% [ Z Vp Vs density ]. The code wants the metrics to be in km/s.
addpath '../../sub';mkdir('plots');mkdir('mat');
clear all;
close all;
clc;
...
typ = 't' ;          % 't': enters time series length (win= in s) and rate (rate=1/s)
                     ...
                     % (1/Hz)
win = 500;           % (in s or Hz) window length
rate = 0.05 ;         % (in 1/s or Hz) sampling rate
FT = make_ft(win,rate,typ);
FT.Fmax=2;         % calculate until Fmax
max_mode=1;         % maximum number of surface-wave modes to ouput
freq=(0.1:0.1:5);     % plots eigenfunctions for periods 1 -> 10s


%% MEDIUM
Dmax=1000;
x=importdata('./1Dprofile_CI_HEC.dat');
% sort depth by increasing order:
% what metrics are we working on:
% convert depth from m g to km, kg
% convert rho, Vp, Vs from m/s to km/s and 
% fac=1;if min(x.data(2:end-1,3))<1000;fac=1E-3;end
fac=1E-3;
% if x(2,3)>=x(:,2)
% MED(1).z       = flipud(x(2:end,3))*fac;
% MED(1).alpha   = flipud(x(2:end,4))*fac;
% MED(1).beta    = flipud(x(2:end,5))*fac;
% MED(1).rho     = flipud(x(2:end,6))*fac;
% else
% MED(1).z       = x.data(:,3)*fac;
% MED(1).alpha   = x.data(:,4)*fac;
% MED(1).beta    = x.data(:,5)*fac;
% MED(1).rho     = x.data(:,6)*fac;
[~,ii]=sort(x(:,3),'descend');
MED(1).z       = x(ii,3)*fac;
MED(1).alpha   = x(ii,4)*fac;
MED(1).beta    = x(ii,5)*fac;
MED(1).rho     = x(ii,6)*fac;

% end
plot(MED(1).beta,-MED(1).z,'b+-',MED(1).alpha,-MED(1).z,'r+-',MED(1).rho,-MED(1).z,'k+-');grid on
ylim([-50 0])
print('-dpsc','./plots/profile_plots.ps');

%% get interfaces:
% At this point, you have to mark where the interfaces are. The problem is
% piece-wise smooth: kinks and sharp discontinuities may bring issues!
% for kinks: pick the *exact* depth of the kink
% for discontinuities: pick a depth between the two values.
% There is a subroutine that can interactively pick the interfaces:
% MED(1).inter = get_layers(MED(1).z ,MED(1).alpha,MED(1).beta,MED(1).rho);
%%% make sure you familiarize yourself with the commands by clicking 
%%% >> help get_layers
% You can also pick them by eye and write them in descending order in the
% following vector:
MED(1).inter = [ 31.575 16.48 15.48 6 5 0.78 0.04 0]; % chn
return
% MED(1).inter = [30.75 16.6166 15.4154 6.006 5.005 1.001 0]; % hec
% MED(1).alpha=MED(1).alpha(ii);
% MED(1).beta=MED(1).beta(ii);
% MED(1).rho=MED(1).rho(ii);
MED(1).Nl=length(MED(1).inter);MED(1).solid=1;MED(1).Dmax=Dmax;
% definte property in each layer:
for i=2:MED(1).Nl
    MED(i).solid=1;
    MED(i).typ='';  % placeholder for constant or continuous (not applied in arbitrary case)
    ik=find(MED(1).inter(i-1)>=MED(1).z&MED(1).inter(i)<=MED(1).z);
    MED(i).betal = (MED(1).beta(ik));
    MED(i).alphal = (MED(1).alpha(ik));
    MED(i).rhol = (MED(1).rho(ik));
    MED(i).zz = (MED(1).z(ik));
    MED(i).zz(1) = max([MED(1).inter(i)  MED(1).inter(i-1)]);
    MED(i).zz(end) = min([MED(1).inter(i)  MED(1).inter(i-1)]);
end
plot(MED(1).beta,-MED(1).z,'b',MED(1).alpha,-MED(1).z,'r',MED(1).rho,-MED(1).z,'k');grid on
ylim([-50 0])
print('-dpsc',['./plots/profile.ps']);
...
NR.N1 = 25 ;        % lower bound for number of points / layer
NR.N2 = 70 ;        % maximum number of points / layers


%% GESC
bigC = gesc(MED,FT,NR,max_mode,freq);
save(['./mat/air_arbitrary_CHN'],'bigC','NR','MED','FT','-v7.3')
load ./mat/air_arbitrary_CHN
